...
- (void)test {
    printf("CAJAGOOGOGO");
//    SETS POINT ON SCREEN OF FOCUS
//    _captureDevice.focusPointOfInterest = CGPointMake(-1.0,  1.0);
    
 
//    _captureDevice.lensPosition = 0.3; //READONLY
}
...
- (void)setAutoFocus:(BOOL)enable
{
    ...
    if (enable) {
        printf("helloAUTO TRUE");
    }else{
        printf("helloAUTO FALSE");
    }
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  }else if ([@"setAutoFocus" isEqualToString:call.method]){
      // NSNumber *step = call.arguments[@"step"];
      // [_camera zoom:[step doubleValue]];
      // result(nil);
